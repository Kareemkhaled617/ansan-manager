...
import '../../common/theme/system_ui_overlay_style.dart';
...
class VideosScreen extends StatelessWidget {
  const VideosScreen({
    required this.styleVersions,
    super.key,
  });

  final List<String> styleVersions;
...
class _VideoScreen extends StatefulWidget {
  const _VideoScreen({required this.styleVersions});
...
class __VideoScreenState extends State<_VideoScreen> with AppBarMixin {
  ...
  void _onDetectToUpdateStatusBar(VisibilityInfo info) {
    ...
    if (info.visibleFraction == 1) {
      final systemUiOverlayStyle = getSystemUiOverlay(appModel.darkTheme);
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              gradient: LinearGradient(
                                ...
                                colors: <Color>[
                                  Colors.transparent,
                                  const Color(0xFF000000).withValueOpacity(0.2),
                                  const Color(0xFF000000).withValueOpacity(0.3),
                                ],
  ...
  Widget _renderVideoPage(VideosModel model, int index, double maxHeight) {
    ...
    final styleVersion =
        widget.styleVersions[index % widget.styleVersions.length];
