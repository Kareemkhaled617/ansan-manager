...
class VendorAdminService extends WooCommerceService {
  ...
  VendorAdminService({
    ...
    required String platform,
        ...
        _consumerKey =
            consumerKey.contains('ck_') ? consumerKey : 'ck_$consumerKey',
        _consumerSecret = consumerSecret.contains('cs_')
            ? consumerSecret
            : 'cs_$consumerSecret';
      ...
      if (fileImages.isNotEmpty) {
        for (var i = 0; i < fileImages.length; i++) {
          if (fileImages[i] is! String ||
              fileImages[i]?.toString().isURL == false) {
            final base64Data = await ImageTools.compressImage(
              fileImages[i],
              percentage: 100,
              quality: 100,
            );
          ...
          } else if (fileImages[i] is String &&
              fileImages[i]?.toString().isURL == true) {
  ...
  Future<Product> createVendorAdminProduct({
    ...
    try {
      ...
      if (featuredImage != null) {
        preparedFeaturedImage = await ImageTools.compressImage(
          featuredImage,
          percentage: 100,
          quality: 100,
        );
  ...
  Future<int> updateInfo(cookie, {firstName, lastName, phone, avatar}) async {
    try {
      ...
      printLog('vendor_admin.dart updateInfo: $e');
      return -1;
    }
    return 1;
  }
