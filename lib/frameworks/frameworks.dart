...
import '../screens/detail/widgets/product_price/yith_addons_price.dart';
...
abstract class BaseFrameworks {
  ...
  bool enableShoppingCart(Product? product) =>
      kAdvanceConfig.enableCart && product?.isListing != true;
  ...
  Widget renderTaxes(TaxModel taxModel, BuildContext context) {
    ...
    final taxes = taxModel.taxes;
    final taxesTotal = taxModel.taxesTotal;
  ...
  Widget renderShippingMethodInfo(BuildContext context,
    ...
    return cartModel.isEnabledShipping() && cartModel.shippingMethod != null
        ? Padding(
            padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 20),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              ...
              children: <Widget>[
                Expanded(
                  child: Services().widget.renderShippingPaymentTitle(
                        context,
                        cartModel.shippingMethod!.title.toString(),
                      ),
                ),
                Text(
                  PriceTools.getCurrencyFormatted(
                      cartModel.getShippingCost(includeTax: true), currencyRate,
  ...
  Widget renderProductCardView({
    ...
    final cardDesign = config.cardDesign;
    ...
    switch (cardDesign) {
      ...
      case CardDesign.simpleForWeb:
      default:
        if (cardDesign.isSimpleForWeb && kIsWeb) {
  ...
  /// render yith addons options cart item
  ...
  Future<void> openExternal(BuildContext context, Product product) async {
    ...
    if (url != null) {
      try {
        if (kIsWeb || Tools.needToOpenExternalApp(url)) {
  ...
  Widget renderDetailPrice(
    ...
    if (product.yithAddOns?.isNotEmpty ?? false) {
      var model = context.read<ProductVariantModel>();
      if (model.selectedYithOptions.isNotEmpty) {
        return YithAddonsPrice(
          product: product,
          priceData: priceData,
        );
      }
  ...
  Widget renderLoyaltyCouponInfo(BuildContext context) {
    final currencyRate =
        Provider.of<AppModel>(context, listen: false).currencyRate;
    final currency = Provider.of<AppModel>(context, listen: false).currency;
    final loyaltyCoupon =
        Provider.of<CartModel>(context, listen: false).loyaltyCoupon;
      ...
      return Padding(
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            Text(
              PriceTools.getCurrencyFormatted(
                  loyaltyCoupon.value ?? 0, currencyRate,
                  currency: currency)!,
              style: Theme.of(context).textTheme.titleMedium!.copyWith(
                    fontSize: 14,
                    color: Theme.of(context).colorScheme.secondary,
  ...
  /// Check if the user info is valid for shipping
  /// This is a temporary function to bypass the Shopify validation
