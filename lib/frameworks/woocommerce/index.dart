...
class WooWidget extends BaseFrameworks
  ...
  Future<void> applyCoupon(
    ...
    try {
      ...
      error!(err.toString().clearExceptionKey().unescape());
      return;
    }
  ...
  Widget renderVariantItem(
    ...
    final color = HexColor(context.getHexColor(option));
      ...
      if (name == 'color') {
        return Container(
          ...
          decoration: BoxDecoration(
            border: Border.all(
              color:
                  Theme.of(context).colorScheme.secondary.withValueOpacity(0.3),
            ),
            color: color,
          ),
        );
...
//    final cartModel = Provider.of<CartModel>(context, listen: false);
  ...
  String? getPriceItemInCart(
    ...
    if (hasYithAddons) {
      ...
      var total = YithAddonsTools.getSelectedYithOptionsPrice(
              basePrice, cartItemMetaData!.selectedYithOptions!, quantity) +
          basePrice * quantity;

      return PriceTools.getCurrencyFormatted(total, currencyRate,
  ...
  Future<bool>? syncCartFromWebsite(
    ...
    if (kAdvanceConfig.enableSyncCartFromWebsite == false) {
      return false;
    }
