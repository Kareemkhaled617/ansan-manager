...
class NavigateTools {
    ...
    if (config.hasUrl) {
      ...
      await FluxNavigate.pushNamed(
        ...
        arguments: BackDropArguments(
          config: config,
          data: products,
          categoryMenuStyle: ProductCategoryMenuStyle.fromString(
  ...
  static void navigateToProductDetail(
    ...
    /// Apply `openInWebview` config from env if `config` is null
    final productConfig = config ?? ProductConfig.empty();
    if (productConfig.openInWebview) {
