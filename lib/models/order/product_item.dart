...
class ProductItem {
  ...
  ProductItem.customerAccountShopify(Map parsedJson) {
    try {
      productId = parsedJson['productId'];
      id = parsedJson['id'];
      name = parsedJson['title'];
      quantity = num.tryParse('${parsedJson['quantity']}')?.toInt() ?? 1;
      // total = parsedJson['variant']?['price']?['amount'];
      total = parsedJson['totalPrice']?['amount'];
      featuredImage = parsedJson['image']?['url'];
    } catch (e, trace) {
      printLog(e.toString());
      printLog(trace.toString());
    }
  ...
  ProductItem.fromFirebaseJson(Map parsedJson) {
    try {
      productId = parsedJson['productId'].toString();
      quantity = double.tryParse('${parsedJson['quantity']}')?.round() ?? 0;
      total = parsedJson['price'];
      featuredImage = parsedJson['image'];
      name = parsedJson['title'] ?? '';
      featuredImage ??= kDefaultImage;
      variationId = parsedJson['variationId'];
      final selectedVariation = parsedJson['selectedVariation'];
      if (selectedVariation is List) {
        for (var item in selectedVariation) {
          final name = item['name'];
          final value = item['option'];
          if (name != null && value != null) {
            attributes.add(name);
            addonsOptions[name] = value;
          }
        }
      }
    } catch (e, trace) {
  ...
  LiteProduct toLiteProduct(String orderId) {
    return LiteProduct(
      id: productId ?? '',
      name: name ?? '',
      image: featuredImage ?? kDefaultImage,
