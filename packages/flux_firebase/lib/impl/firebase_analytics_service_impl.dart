...
class FirebaseAnalyticsServiceImpl extends FirebaseAnalyticsService {
  ...
  FirebaseAnalyticsServiceImpl({FirebaseApp? app}) {
    analytics = app != null
  ...
  Future<void> logCustomEvent({
    ...
    await analytics.logEvent(name: name, parameters: parameter);
  }
    ...
    if (data is List<Product>) {
      ...
      for (var item in data) {
        items.add(AnalyticsEventItem(
          itemId: item.id,
          itemName: item.name,
          itemCategory: item.type ?? item.categoryId,
    ...
    if (data is Product) {
      items = [
        AnalyticsEventItem(
          itemId: data.id,
          itemName: data.name,
          itemCategory: data.type ?? data.categoryId,
      ...
      for (var item in data) {
        if (item is ProductItem) {
          items.add(AnalyticsEventItem(
            itemId: item.id,
            itemName: item.name,
            itemCategory: item.product?.type ?? item.product?.categoryId,
